...
public class FluidProcessesTests : IDisposable
{
    ...
    private static readonly Pressure PressureDrop =
        50.Kilopascals();
            ...
            .Should().Be(
                _fluid.WithState(
                    ...
                    Input.Enthalpy(
                        _fluid.Enthalpy - (
                            _fluid.Enthalpy -
                            _fluid.IsentropicExpansionTo(LowPressure).Enthalpy
                        ) * IsentropicEfficiency.DecimalFractions
                    )
                )
            );
    ...
    public void CoolingTo_TemperatureWithPressureDrop_ReturnsFluidAtTemperatureAndLowerPressure() =>
        _fluid.CoolingTo(
            _fluid.Temperature - TemperatureDelta,
            PressureDrop
        ).Should().Be(
            _fluid.WithState(
    ...
    )
    {
        Action action = () =>
            _ = _fluid.CoolingTo(
                _fluid.Enthalpy +
                enthalpyDelta.KilojoulesPerKilogram(),
                pressureDrop.Kilopascals()
            );
        action.Should().Throw<ArgumentException>()
            .WithMessage($"*{message}*");
    ...
    {
        var first = new Fluid(FluidsList.Ammonia)
            .DewPointAt(1.Atmospheres());
        var second = _fluid.HeatingTo(
            _fluid.Temperature + TemperatureDelta
        );
        Action action = () =>
            _ = _fluid.Mixing(
