...
public abstract partial class AbstractFluid
{
    ...
    public double? Compressibility =>
        _compressibility ??=
            NullableKeyedOutput(Parameters.iZ);
    ...
    public KinematicViscosity? KinematicViscosity =>
        ...
        .ToUnit(KinematicViscosityUnit.Centistokes);
    ...
    public Temperature MaxTemperature =>
        _maxTemperature ??=
            Temperature.FromKelvins(
                KeyedOutput(Parameters.iT_max)
            ).ToUnit(TemperatureUnit.DegreeCelsius);
    ...
    public Phases Phase =>
        _phase ??=
            (Phases) KeyedOutput(Parameters.iPhase);
    ...
    public double? Prandtl =>
        _prandtl ??=
            NullableKeyedOutput(Parameters.iPrandtl);
    ...
    public Ratio? Quality =>
        _quality ??=
            KeyedOutputIsNotNull(Parameters.iQ, out var output)
                ? Ratio.FromDecimalFractions(output!.Value)
    ...
    ///     specific heat (by default, kJ/kg/K).
    /// </summary>
    public SpecificEntropy SpecificHeat =>
        _specificHeat ??=
            SpecificEntropy.FromJoulesPerKilogramKelvin(
                KeyedOutput(Parameters.iCpmass)
            ).ToUnit(SpecificEntropyUnit.KilojoulePerKilogramKelvin);
    ...
    public Temperature Temperature =>
        _temperature ??=
            Temperature.FromKelvins(
                KeyedOutput(Parameters.iT)
            ).ToUnit(TemperatureUnit.DegreeCelsius);

    /// <summary>
    ///     Absolute pressure at
    ///     the triple point (by default, kPa).
    /// </summary>
